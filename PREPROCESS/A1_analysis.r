...
sampleDir <- "/mnt/rdisk/duydao/SPATIAL/GeneSmart/output/A1/HTI_spaceranger_count_A1/outs"

## CUSTOM READVISIUM
source("/mnt/rdisk/duydao/SPATIAL/GeneSmart/scripts/bin/readVisium_duy.r")
...
p <- qPlot(sce)
p
ggsave(file=paste(fig_path,"qPlot_A1.png"),p, width=2400, height=3000, unit="px", dpi=300)
...
p <- clusterPlot(sce, palette=color, color="black", size=0.1) +
    labs(title="A1 Clusters")
p
ggsave(paste(fig_path,"cluster_q13_d15_i50K.png"), p, width=3000, height=3000, unit="px", dpi=300)


# SAVE DATA
rdata <- "/mnt/d4t/SPATIAL/GeneSmart/rdata/"

saveRDS(sce, file = paste(rdata,"spatialA1_q13_ready.rds"), ascii = FALSE, version = NULL,
...
saveRDS(sce.enhanced, file = paste(rdata,"spatialA1_q13_enhanced.rds"), ascii=FALSE, version=NULL,
        compress=TRUE, refhook=NULL)
## Import sce.enhanced
sce.enhanced <- readRDS("/mnt/d4t/SPATIAL/GeneSmart/rdata/ spatialA1_q13_enhanced.rds")
...
p
ggsave(paste(fig_path,"cluster_A1_enhanced_q13_i50K.png"), p, width=3000, height=3000, unit="px", dpi=300)
...
enhanced.plots <- purrr::map(
    markers[["HER_MET"]], function(x) featurePlot(sce.enhanced, x) + 
    viridis::scale_fill_viridis(option="C") # or K
    )
p <- patchwork::wrap_plots(enhanced.plots, nrow=1, ncol=2)
p
ggsave(paste(fig_path,"markers_HER_MET_enhanced.png"), p, width=3000, height=1500, unit="px", dpi=300)
...
## save data
saveRDS(sce.enhanced, file = paste(rdata,"spatialA1_q13_enhanced_2000hvgs.rds"), ascii = FALSE, version = NULL,
        compress = TRUE, refhook = NULL)
...
p<-Seurat::DoHeatmap(sobj, features = top_markers$gene, slot='scale.data',
  ...
  guides(col = FALSE)
p
ggsave(paste(fig_path,"DEGs_q13.png"), p, width=2400, height=3000, unit="px", dpi=300)
