...
# PATH
fig_path <- "/mnt/rdisk/duydao/SPATIAL/GeneSmart/figures/A1/spaCET/"
rdata <- "/mnt/rdisk/duydao/SPATIAL/GeneSmart/rdata/A1/spaCET/"
visiumPath <- "/mnt/rdisk/duydao/SPATIAL/GeneSmart/output/A1/HTI_spaceranger_count_A1/outs/"
...
saveRDS(
        SpaCET_obj,
        file          = paste0(rdata, "A1_LUAD_spaCET_obj.rds"), 
        ascii         = FALSE, 
        version       = NULL,
        compress      = TRUE, 
        refhook       = NULL
...
SpaCET_obj <- readRDS(paste0(rdata,"A1_LUAD_spaCET_obj.rds"))
...
ggsave(paste0(fig_path,"A1_cellType.png"), p2, width=3500, height=1500, unit="px", dpi=300)
...
ggsave(paste0(fig_path,"A1_AllCellType.png"), p3, width=3500, height=3000, unit="px", dpi=300)
...
ggsave(paste0(fig_path,"interaction_colocalized_pDC_Endothelial.png"), p6, width=5500, height=2000, unit="px", dpi=300)
###
...
SpaCET_obj <- SpaCET.combine.interface(SpaCET_obj, cellTypePair = c("pDC","Endothelial"))
...
p8 <- SpaCET.visualize.spatialFeature(
                      SpaCET_obj, 
                      spatialType = "Interface", 
                      spatialFeature = "Interface&Endothelial_pDC"
                      )
ggsave(paste0(fig_path,"interface_pDC_Endothelial.png"), p8, width=3000, height=3000, unit="px", dpi=300)

## Compute the distance of CAF-M2 to tumor border
p9 <- SpaCET.distance.to.interface(
                      SpaCET_obj, 
                      cellTypePair = c("Endothelial","pDC")
                      )
ggsave(paste0(fig_path,"distance_pDC_Endo_tumorBorder.png"), p9, width=3000, height=3000, unit="px", dpi=300)
